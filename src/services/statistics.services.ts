...
function buildRatingQuery(filters: SurveyStatisticsFilters = {}) {
  ...
  if (includeOnline) {
    selects.push(`
      SELECT
        sf.id::text AS form_id,
        sf.code AS form_code,
        sf.title AS form_title,
        ss.id::text AS section_id,
  ...
  if (includeHardcopy) {
    selects.push(`
      SELECT
        ...
        si.statement AS item_statement,
        si.sort_order AS item_sort_order,
        CONCAT('hardcopy:', mb.id, ':', si.id::text, ':', mac.rating, ':', hardcopy_row.answer_index) AS response_id,
